...
const Header = () => {
    ...
    const handleStickyHeader = () => {
        if (document.body.scrollTop > 80 || document.documentElement.scrollTop > 80) {
            headerRef.current.classList.add('stiky__header')
        } else {
                            ...
                            token && user ?
                                <div>
                                    <Link to={`${role === 'doctor' ? '/doctors/profile/me' : '/users/profile/me'}`}>
                                        <figure className="w-[35px] h-[35px] rounded-full border-2 border-solid border-primaryColor cursor-pointer ">
                                            <img src={user?.photo} alt="anh dai dien" className="w-full h-full rounded-full"/>
                                        </figure>
                                    </Link>
                                    {/* <h2>{user?.name}</h2> */}
                                </div> :
